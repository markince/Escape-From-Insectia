...
public class RedGemPickup : MonoBehaviour
{
    [SerializeField] AudioClip redGemPickupSFX = null;
    [SerializeField] int pointsPerRedGemPickup = 500;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision == playerBody)
        {
            ...
            FindObjectOfType<GameSession>().IncrementRedGems(pointsPerRedGemPickup);

            // PLay gem pickup sound
            PlayAudioClipAtPoint(Camera.main.transform.position, 0.0F, redGemPickupSFX);

            CombatTextManager.Instance.CreateText(transform.position, "+" + pointsPerRedGemPickup.ToString(), Color.yellow);
