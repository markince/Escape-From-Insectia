...
public class GreenGemPickup : MonoBehaviour
{
    [SerializeField] AudioClip greenGemPickupSFX = null;
    [SerializeField] int pointsPerGreenGemPickup = 100;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision == playerBody)
        {
            ...
            FindObjectOfType<GameSession>().IncrementGreenGems(pointsPerGreenGemPickup);

            CombatTextManager.Instance.CreateText(transform.position, "+" + pointsPerGreenGemPickup.ToString(), Color.yellow);


            // PLay gem pickup sound
            PlayAudioClipAtPoint(Camera.main.transform.position, 0.0F, greenGemPickupSFX);
